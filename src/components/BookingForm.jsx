...
const BookingForm = ({ user, pond, selectedSeats, payType, receiptData, onSetPayType, onHandleReceiptChange, onClearReceipt, onSubmitBooking, onOpenAuth }) => {
    ...
    if (!user) {
        return (<div className="card booking-form-card">
    ...
    return (<div className="card booking-form-card">
      ...
      <div className="selected-pills">
        {selectedSeats.length ? (selectedSeats.map(n => <span key={n} className="seat-pill">#{n}</span>)) : (<div style={{ fontSize: '11px', color: 'var(--muted)' }}>No pegs selected yet</div>)}
      </div>
      ...
      <div className="upload-zone" onClick={() => fileInputRef.current?.click()} onDragOver={handleDragOver} onDrop={handleDrop}>
        <input ref={fileInputRef} type="file" id="receipt-file" accept="image/*,application/pdf" onChange={handleFileChange} style={{ display: 'none' }}/>
      ...
      {receiptData && (<div className="upload-preview">
      ...
      <textarea className="form-input" rows={2} placeholder="Any special requests..." value={notes} onChange={(e) => setNotes(e.target.value)}/>
      <button id="btn-submit" className="btn btn-primary w-full btn-lg mt-3" onClick={onSubmitBooking} disabled={!selectedSeats.length || !receiptData}>
