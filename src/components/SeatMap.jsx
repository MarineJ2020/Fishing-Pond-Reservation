...
const SeatMap = ({ pond, selectedSeats, onToggleSeat }) => {
    const zones = useMemo(() => {
        if (!pond)
            return [];
    ...
    if (!pond) {
        return (<div className="card seat-map-card">
    ...
    const handleSeatClick = (num, status) => {
    ...
    return (<div className="card seat-map-card">
      <div className="seat-map-header">
        ...
        <div className="seat-legend">
          ...
          <div className="legend-item">
            <div className="legend-dot pending"></div> Pending
          </div>
          <div className="legend-item">
            <div className="legend-dot booked"></div> Taken
          </div>
      ...
      <div id="seat-map-wrap" className="seat-map-wrap">
        <div className="seat-zone-wrap left">
          ...
          <div className="seat-col">
            ...
            const cls = isBooked || isPending ? (isPending ? 'pending' : 'booked') : isSelected ? 'selected' : 'avail';
            return (<button key={s.num} className={`seat-btn ${cls}`} onClick={() => handleSeatClick(s.num, s.status)} disabled={isBooked || isPending} title={`Peg #${s.num} - ${s.status}`} type="button">
                    {s.num}
                  </button>);
        })}
          </div>
      ...
      {selectedSeats.length > 0 && (<div className="price-bar">
          <div>
