...
import BookingReceivedEmail from '../../lib/emails/booking-received';
import BookingApprovedEmail from '../../lib/emails/booking-approved';
import BookingRejectedEmail from '../../lib/emails/booking-rejected';
...
app.post('/createClientAccount', verifyToken, requireStaff, async (req, res) => {
    ...
    try {
        ...
        const html = renderToStaticMarkup(<WelcomeEmail name={name} email={email} appUrl={process.env.APP_URL || 'https://your-app-url.com'}/>);
        await sendEmail({
            to: email,
...
app.post('/createBooking', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const html = renderToStaticMarkup(<BookingReceivedEmail bookingRef={bookingRef} amount={amount} appUrl={process.env.APP_URL || 'https://your-app-url.com'}/>);
...
app.post('/approveBooking', verifyToken, requireStaff, async (req, res) => {
    ...
    try {
        ...
        const html = renderToStaticMarkup(<BookingApprovedEmail bookingRef={booking?.bookingRef || ''} appUrl={process.env.APP_URL || 'https://your-app-url.com'}/>);
        if (userRecord?.email) {
            await sendEmail({
                to: userRecord.email,
...
app.post('/rejectBooking', verifyToken, requireStaff, async (req, res) => {
    ...
    try {
        ...
        const userId = booking.userId?.id;
        const userRecord = userId ? await adminAuth.getUser(userId) : null;
        const html = renderToStaticMarkup(<BookingRejectedEmail bookingRef={booking?.bookingRef || ''} appUrl={process.env.APP_URL || 'https://your-app-url.com'}/>);
        if (userRecord?.email) {
            await sendEmail({
                to: userRecord.email,
